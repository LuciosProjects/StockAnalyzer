...
class C_trader_traits:
    ...
    def __init__(self, Data):
        ...
        # distribution = {sector: np.random.rand() for sector in Data.Market.Sectors}
        # distribution_sum = sum(distribution[sector] for sector in Data.Market.Sectors)

        # self.sectorBiases = {sector: distribution[sector]/distribution_sum for sector in Data.Market.Sectors}
        # ''' Personal bias about each sector '''

        # distribution = {region: np.random.rand() for region in Data.Market.Regions}
        # distribution_sum = sum(distribution[region] for region in Data.Market.Regions)

        # self.geographicalBiases = {region: distribution[region]/distribution_sum for region in Data.Market.Regions}
        # ''' Personal bias about each region '''
...
class Trader:
    def __init__(self, Inp, Data) -> None:
        ...
        if(class_idx < Data.Market.wealthThresholds.__len__()):
            minWealth = Data.Market.wealthThresholds[class_idx-1] if class_idx > 0 else 0
            maxWealth = Data.Market.wealthThresholds[class_idx]
        else:
            minWealth = Data.Market.wealthThresholds[-1]

            wealthyCap = Data.Market.totalMarketCap
            for i in range(0,Data.Market.wealthDistribution.__len__()-1):
                wealthyCap -= Inp.wealthDistribution[i]*Data.Market.traderPoolSize*Data.Market.wealthThresholds[i]

            maxWealth = wealthyCap/(Data.Market.traderPoolSize*Inp.wealthDistribution[-1])

        self.balance        = rnd.uniform(minWealth,maxWealth)
        ''' Avaialable cash a trader has to make trades '''

        self.income         = self.calculate_income(Data, 'Init')
        ...
        self.Portfolio = {ticker: {'Holdings': 0, 'ask': 0, 'ask_amount': 0, 'bid': 0, 'bid_amount': 0, 'Active': Data.Companies[ticker].Active} for ticker in Data.Market.tickers}
        ''' Trader's portfolio, shows each stock in the market, how many holdings the trader has for each stock,\n
            asking price (if wants to buy) and selling price (if has any holdings and wants to sell) '''
        
        self.numHoldingTypes = 0
        ''' Number of different types of stocks the trader has holdings for '''
    ...
    def trading_day(self, Data, mode='Reg'):
        ...
        self.numHoldingTypes = sum(self.Portfolio[ticker]['Holdings'] > 0 for ticker in self.Portfolio)
    ...
    def _trading_day_init(self, Data):
        ...
        for ticker in Data.Market.tickers:
                ...
                total_shares = company.availableShares
                remaining_shares = total_shares

                if(self.Portfolio[ticker]['Active']):
                    ticker_descision = self._make_descision(company, Data.Market)
                else:
                    continue

                if ticker_descision == 'buy':
                    self.place_buy_order(ticker, Data)
                pass
    ...
    def _trading_day(self, Data):
        ...
        for ticker in Data.Market.tickers:
                ...
                if(self.Portfolio[ticker]['Active']):
                    amount, ticker_descision = self._make_descision(company, Data.Market)
                else:
                    continue

                pass
    
    def _make_descision(self, Company: Company, Market) -> tuple:
        ''' 
            Determines if the trader wants to buy, sell or hold a stock
        '''

        if (self.balance <= Company.price):
            # In case the trader has no money, they cannot buy
            return (0, 'hold')
        ...
        diversification = self.traits.strategy_oriented.Diversification

        # information_advantage = self.traits.environmental_influences.information_advantage # TBD

        # Calculate probabilities based on traits and company/market factors
        buy_probability = self.traits.securityBiases[ticker]*(  self.traits.personality_based.risk_appetite*Company.volatility_index + \
                                                                self.traits.environmental_influences.popularity_dependence*Company.popularity + \
                                                                Company.growth_potential) / 3

        # Market sentiment factor
        market_sentiment_factor = self.traits.environmental_influences.market_sentiment * \
                                (Market.marketTrendPosition["World"] + \
                                 Market.marketTrendPosition["Sectors"][sector] + \
                                 Market.marketTrendPosition["Geogarphical"][region])/ 3.0

        # Combine influences based on the trader's preference
        influences = np.array([Company.fundamental_influence, Company.technical_influence])/(Company.fundamental_influence + Company.technical_influence)
        combined_influence = (self.traits.strategy_oriented.technical_vs_fundamental_analysis * influences[0] +
                              (1 - self.traits.strategy_oriented.technical_vs_fundamental_analysis) * influences[1])

        # Diversification factor
        diversification_factor = self.traits.strategy_oriented.Diversification * (1.0 - self.numHoldingTypes/Market.numTickers)

        buy_probability *= (1 + combined_influence) * (1 + market_sentiment_factor) * (1 + diversification_factor)

        if(np.random.rand() <= buy_probability):
            # determine how much to buy (TBD)
            amount = 0
            return (amount, 'buy')
        else:   
            # if trader has no holdings, they cannot sell
            if(self.Portfolio[ticker]['Holdings'] == 0):
                return (0, 'hold') # if 'hold' is returned, the trader will not make any trades, so the amount is irrelevant
            else:
                # determine if trader wants to sell
                sell_probability = (1 - self.traits.personality_based.risk_appetite + \
                                    self.traits.behavioral_based.herd_mentality + \
                                    self.traits.behavioral_based.rationality + \
                                    (1 - Company.growth_potential) + (1 - Company.popularity)) / 5
    ...
    def place_buy_order(self, ticker, Data):
        company = Data.Companies[ticker]
        shares_to_buy = self.Portfolio[ticker]['bid_amount']
        bid_price = self.Portfolio[ticker]['bid']
        shares_to_buy = min(shares_to_buy, company.availableShares)

        self.Portfolio[ticker]['Holdings'] += shares_to_buy
        self.balance -= shares_to_buy * company.price
        company.availableShares -= shares_to_buy

    def place_sell_order(self, ticker, Data):
        company = Data.Companies[ticker]
        shares_to_sell = self.Portfolio[ticker]['Holdings']

        self.Portfolio[ticker]['Holdings'] -= shares_to_sell
        self.balance += shares_to_sell * company.price
        company.availableShares += shares_to_sell
